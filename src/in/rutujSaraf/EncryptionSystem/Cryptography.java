package in.rutujSaraf.EncryptionSystem;
...
class SavePPTFileFilter extends FileFilter
 {
    public boolean accept(File f)
   {
        if (f.isDirectory())
...
class SavePDFFilter extends FileFilter
 {
    public boolean accept(File f)
   {
        if (f.isDirectory())
...
class SaveImgFileFilter extends FileFilter
 {
    public boolean accept(File f)
   {
        if (f.isDirectory())
...
class SaveWordFileFilter extends FileFilter
 {
    public boolean accept(File f)
   {
        if (f.isDirectory())
...
int wdth,hight;
    ...
    public Cryptography() {
        ...
        setResizable(false);
        label = new JLabel("CHOOSE FILE FOR ENCRYPTION OR DECRYPTION");
		...
		buttonPanel1.setForeground(Color.green);
...
loadPanel.add(im);
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	convert(-key);
    		...
    		}catch(HeadlessException | IOException ioee){
    		}
    	}
    	if(e.getSource()==denc)
    	{try
...
jFileChooser.addChoosableFileFilter(new SaveImgFileFilter());
jFileChooser.addChoosableFileFilter(new SavePDFFilter());
jFileChooser.addChoosableFileFilter(new SaveWordFileFilter());
jFileChooser.addChoosableFileFilter(new SavePPTFileFilter());

jFileChooser.setSelectedFile(new File("fileToSave.txt"));
jFileChooser.setSelectedFile(new File("fileToSave.jpg"));
jFileChooser.setSelectedFile(new File("fileToSave.pdf"));
jFileChooser.setSelectedFile(new File("fileToSave.docx"));
jFileChooser.setSelectedFile(new File("fileToSave.pptx"));
